...
Goal: metadata + speech transcription for audio/video. **Done (no cgo)** via an `ffprobe` metadata block (format/duration/streams) in `media.go`, with an optional `whisper` transcript hook. ffmpeg/ffprobe + **whisper verified end-to-end** (espeak-ng → whisper; the transcription test is gated behind `DISTILL_TEST_WHISPER` since it loads a model).
